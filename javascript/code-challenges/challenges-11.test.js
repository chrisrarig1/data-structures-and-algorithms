...
function transformToLis(obj){
  ...
  for( let i=0; i < keys.length; i++){
    newArr.push(`<li>${keys[i]}: ${val[i]}</li>`);
  }
  return newArr;
}
...
const totalSum = (input) => {
  return input.reduce((acc, arr) => acc += arr.reduce((accum, value) => accum += value), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = [];
  input.filter((acc, val) => {
    if(val % 5 === 0 ){
      newArr.push(val);
    }}, 0);
  let final = newArr.map(el => {
...
let findMaleAndFemale = (data) => {
  let mF = data.filter(star => star.gender === 'male' || star.gender === 'female');
  let names = mF.map(el => {
    return el.name;
  });
